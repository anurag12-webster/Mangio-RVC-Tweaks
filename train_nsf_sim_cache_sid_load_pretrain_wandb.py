...
def run(rank, n_gpus, hps):
    ...
    if rank == 0:
        ...
        wandb.init(
            project=hps.name,  # Use the experiment name as the project name
            name=f"{hps.name}_run_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config=vars(hps)  # Log all hyperparameters
        )
    ...
    try:  # Automatic resume if possible
    ...
    for batch_idx, info in data_iterator:
        ...
        with autocast(enabled=fp16_run):
            ...
            if rank == 0 and global_step % hps.train.log_interval == 0:
                ...
                # Adjust losses for display
                loss_mel_display = min(loss_mel.item(), 75)
                loss_kl_display = min(loss_kl.item(), 9)

                scalar_dict = {
                    "loss/g/total": safe_item(loss_gen_all),
                    "loss/d/total": safe_item(loss_disc),
                    "learning_rate": lr,
                    "grad_norm_d": grad_norm_d,
                    "grad_norm_g": grad_norm_g,
                    "loss/g/fm": safe_item(loss_fm),
                    "loss/g/mel": loss_mel_display,
                    "loss/g/kl": loss_kl_display,
                    **{"loss/g/{}".format(i): safe_item(v) for i, v in enumerate(losses_gen)},
                    **{"loss/d_r/{}".format(i): safe_item(v) for i, v in enumerate(losses_disc_r)},
                    **{"loss/d_g/{}".format(i): safe_item(v) for i, v in enumerate(losses_disc_g)},
                }
                ...
                for key, image in image_dict.items():
                    wandb_images[key] = wandb.Image(image, caption=key)
